...
//
//   CommentScreen({required this.postId, required this.postOwnerId, this.canReport = false});
//
//   @override
...
//           commentData['postId'] = widget.postId; // Include postId
...
//       Map<String, dynamic> userData = Map<String, dynamic>.from(userSnapshot.value as Map);
//
//       // Update the reference to add comment to the global "comments" node
//       final newCommentRef = FirebaseDatabase.instance.ref().child('comments').push();
//       final newComment = {
//         'content': _commentController.text,
//         'userId': _currentUser!.uid,
//         'postId': widget.postId, // Store postId to link the comment to its post
//         'username': userData['username'] ?? 'Unknown',
//         'userProfileImageUrl': userData['profileImageUrl'] ?? '',
//         'timestamp': DateTime.now().millisecondsSinceEpoch,
//       };
...
//       try {
//         await _commentsRef.child(postId).child('comments').child(comment['commentId']).remove();
...
//                   String postId = comment['postId']; // Ensure 'postId' is included in comment data
//
